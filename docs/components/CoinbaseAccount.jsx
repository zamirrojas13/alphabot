    ...
    <div style={{ padding:'28px 32px', height:'100%', overflowY:'auto', boxSizing:'border-box',
      ...
      {cb && cb.configured && !cb.err && (
        <>
          {/* Balance cards */}
          {(() => {
            const totalFees = (cb.recent_fills || []).reduce((s, f) => s + (f.fee || 0), 0);
            return (
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr 1fr', gap:12, marginBottom:20 }}>
                ...
                <div style={card}>
                  <div style={lbl}>Total Fees (Recent)</div>
                  <div style={{...val, color:'#ff4d6d', fontSize:18}}>
                    -${totalFees.toFixed(4)}
                  </div>
                  <div style={{ fontSize:11, color:'#3e3e52', marginTop:6 }}>
                    from {cb.recent_fills?.length ?? 0} recent fills
                  </div>
                </div>
              </div>
            );
          ...
          {cb.accounts?.length > 0 && (
            <div style={{ background:'#111116', border:'1px solid #1f1f28', borderRadius:10,
              ...
              <table style={{ width:'100%', borderCollapse:'collapse' }}>
                ...
                <tbody>
                  {cb.accounts.filter(a => ['USD','BTC','USDC'].includes(a.currency)).map(a => (
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
            ...
            <div style={{ background:'#111116', border:'1px solid #1f1f28', borderRadius:10, overflow:'hidden' }}>
              ...
              {!cb.recent_fills?.length ? (
                ...
                // FIFO P&L match: each SELL matched to earliest unmatched BUY
                const sorted = [...cb.recent_fills].sort((a, b) => a.time.localeCompare(b.time));
                const buyQ = [];
                const enriched = sorted.map(f => {
                  if (f.side === 'BUY') { buyQ.push(f); return { ...f, pnl: null }; }
                  if (!buyQ.length) return { ...f, pnl: null };
                  const buy = buyQ.shift();
                  const qty = Math.min(f.size, buy.size);
                  const pnl = Math.round(((f.price - buy.price) * qty - f.fee - buy.fee) * 100) / 100;
                  return { ...f, pnl };
                });
                return (
                  <table style={{ width:'100%', borderCollapse:'collapse' }}>
                    <thead>
                      <tr>{['Time','Product','Side','Price','Size','Fee','P&L'].map(h => <th key={h} style={th}>{h}</th>)}</tr>
                    </thead>
                    <tbody>
                      {[...enriched].reverse().map((f, i) => (
                        <tr key={f.trade_id || i} style={{ borderBottom:'1px solid #15151d' }}>
                          ...
                          <td style={{...td, fontWeight:700, color: f.pnl == null ? '#3e3e52' : f.pnl >= 0 ? '#00d084' : '#ff4d6d'}}>
                            {f.pnl == null ? '—' : `${f.pnl >= 0 ? '+' : ''}$${Math.abs(f.pnl).toFixed(2)}`}
                          </td>
                        </tr>
                      ))}
                    </tbody>
