...
// ── message row (compact, expandable) ────────────────────────────────────────
...
function MessageRow({ raw }) {
  ...
  const typeMap = {
    signal: { icon:'⚡', label:'Signal',  color:'#00d084',  bg:'rgba(0,208,132,0.05)',  border:'rgba(0,208,132,0.15)' },
    brief:  { icon:'☀',  label:'Brief',   color:'#a78bfa',  bg:'rgba(167,139,250,0.05)',border:'rgba(167,139,250,0.15)' },
    scan:   { icon:'◎',  label:'Scan',    color:'#5a5a6e',  bg:'rgba(90,90,110,0.04)',  border:'rgba(90,90,110,0.18)' },
    trade:  { icon:'$',  label:'Trade',   color:'#F7931A',  bg:'rgba(247,147,26,0.05)', border:'rgba(247,147,26,0.18)' },
    info:   { icon:'·',  label:'Info',    color:'#3e3e52',  bg:'rgba(62,62,82,0.04)',   border:'rgba(62,62,82,0.15)' },
  };
  const t = typeMap[type];

  return (
    <div onClick={() => setExpanded(e => !e)}
      style={{background: expanded ? t.bg : 'transparent', border:`1px solid ${expanded ? t.border : '#15151d'}`,
        borderRadius:7, padding:'8px 12px', marginBottom:3, cursor:'pointer', transition:'all 0.12s'}}>
      <div style={{display:'flex', gap:10, alignItems:'flex-start'}}>
        <span style={{fontSize:12, color:t.color, flexShrink:0, lineHeight:1.4, minWidth:14, textAlign:'center', marginTop:1}}>{t.icon}</span>
        <div style={{flex:1, minWidth:0}}>
          <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:3, gap:8}}>
            <span style={{fontSize:10, fontWeight:700, color:t.color, letterSpacing:'0.5px', textTransform:'uppercase', flexShrink:0}}>{t.label}</span>
            {ts && <span style={{fontSize:9, color:'#3e3e52', fontFamily:"'JetBrains Mono',monospace", whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>{toEastern(ts)} ET</span>}
          </div>
          <div style={{fontSize:11, color: expanded ? '#ece9e2' : '#5a5a6e', lineHeight:1.55,
            fontFamily:"'JetBrains Mono',monospace",
            whiteSpace: expanded ? 'pre-wrap' : 'nowrap',
            overflow: expanded ? 'visible' : 'hidden',
            textOverflow: expanded ? 'unset' : 'ellipsis',
            wordBreak:'break-word'}}>
            {expanded ? (body || raw) : preview}
          </div>
        </div>
        <span style={{fontSize:9, color:'#2a2a38', flexShrink:0, paddingTop:3}}>{expanded ? '▲' : '▼'}</span>
      </div>
    </div>
  );
}
...
function TelegramPage({ data }) {
  ...
  const lastHeartbeat  = tgData?.last_heartbeat || null;
  // Format heartbeat age for display
  const hbAge = React.useMemo(() => {
    if (!lastHeartbeat) return null;
    try {
      const diff = Math.round((Date.now() - new Date(lastHeartbeat).getTime()) / 1000);
  ...
  return (
    <div style={tgStyles.wrap}>
      ...
      <div style={tgStyles.infoBar}>
        ...
        <div style={{...tgStyles.infoCard, borderColor: connected ? 'rgba(0,208,132,0.2)' : 'rgba(255,77,109,0.15)'}}>
          <div style={tgStyles.infoLabel}>Last Heartbeat</div>
          <div style={{fontSize:18, fontWeight:700, fontFamily:"'JetBrains Mono',monospace", color: connected ? '#00d084' : '#ff4d6d'}}>
            {hbAge || (loading ? '…' : 'Never')}
          </div>
          <div style={{fontSize:10, color:'#3e3e52', marginTop:3}}>
            {connected ? 'Bot scanned within 20 min' : 'No heartbeat in 20+ min'}
          </div>
        </div>
