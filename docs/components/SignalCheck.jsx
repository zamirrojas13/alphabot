...
function SignalCheck({ data, viewMode }) {
  ...
  // Checklist items with priority levels
  // CRITICAL = blocks trade; IMPORTANT = strong warning; ADVISORY = informational
  const checks = sig ? [
    { label:'Weekly alignment',   sub:'Weekly candle must be in bullish structure',         pass: sig.alignment.weekly_ok,                      val: sig.alignment.weekly_ok ? 'Bullish' : 'Not aligned',      priority:'critical' },
    { label:'Daily alignment',    sub:'Daily candle must confirm bullish bias',             pass: sig.alignment.daily_ok,                       val: sig.alignment.daily_ok  ? 'Bullish' : 'Not aligned',      priority:'critical' },
    { label:'R:R meets minimum',  sub:'Reward must be at least the tier minimum',          pass: sig.rr >= config.TIER_RR[sig.tier],           val: `${parseFloat(sig.rr || 0).toFixed(2)} (need ≥${config.TIER_RR[sig.tier]})`, priority:'critical' },
    { label:'No active trade',    sub:'Bot can only hold one position at a time',          pass: !hasActiveTrade,                              val: hasActiveTrade ? 'Trade open' : 'Clear',                   priority:'critical' },
    { label:'H4 alignment',       sub:'4-hour candle must be in uptrend',                  pass: sig.alignment.h4_ok,                          val: sig.alignment.h4_ok    ? 'Bullish' : 'Not aligned',        priority:'important' },
    { label:'Trend condition',    sub:'EMA50 > EMA200 — market is trending up',            pass: sig.conditions.trend,                         val: sig.conditions.trend   ? 'Pass' : 'Fail',                 priority:'important' },
    { label:'Pattern condition',  sub:'The specific candle pattern is confirmed',           pass: sig.conditions.pattern,                       val: sig.conditions.pattern ? 'Pass' : 'Fail',                 priority:'important' },
    { label:'Tier not restricted',sub:'No active loss streak blocking this tier',          pass: !(bot_state.tier_restricted && sig.tier > 1), val: bot_state.tier_restricted && sig.tier>1 ? '⚠ Blocked' : 'Clear', priority:'important' },
    { label:'Extreme condition',  sub:'Price or RSI at an extreme — good entry zone',      pass: sig.conditions.extreme,                       val: sig.conditions.extreme ? 'Pass' : 'Fail',                 priority:'advisory' },
    { label:'Setup type',         sub:'A recognised pattern name was detected',            pass: sig.setup_type && sig.setup_type !== '—',     val: sig.setup_type && sig.setup_type !== '—' ? sig.setup_type : 'None detected', priority:'advisory' },
    { label:'Quality grade',      sub:'A+ / Solid = high historical WR · B = moderate',   pass: ['A+','Solid','B'].includes(sig.grade),        val: sig.grade || '—',                                          priority:'advisory' },
    { label:'Direction',          sub:'Trade direction identified',                         pass: !!sig.direction,                              val: sig.direction?.toUpperCase() || '—',                       priority:'advisory' },
    { label:'Risk tier valid',    sub:'Tier 1–3 assigned based on setup quality',          pass: sig.tier >= 1 && sig.tier <= 3,               val: `T${sig.tier}`,                                            priority:'advisory' },
  ] : [];
      ...
      <div style={scStyles.grid}>
        ...
        <div style={scStyles.card}>
          ...
          {sig ? (
            <>
              ...
              {/* Priority legend */}
              <div style={{display:'flex', gap:10, marginTop:10, marginBottom:4, paddingLeft:2}}>
                {[{p:'critical',c:'#ff4d6d',l:'Critical — blocks trade'},
                  {p:'important',c:'#F7931A',l:'Important — strong warning'},
                  {p:'advisory',c:'#5a5a6e',l:'Advisory — informational'}].map(({p,c,l}) => (
                  <div key={p} style={{display:'flex', gap:4, alignItems:'center'}}>
                    <div style={{width:8, height:8, borderRadius:2, background:c}}></div>
                    <span style={{fontSize:9, color:'#3e3e52'}}>{l}</span>
                  </div>
                ))}
              </div>
              ...
              <div style={{display:'flex', flexDirection:'column', gap:1, marginTop:4}}>
                {checks.map(c => {
                  const failCol = c.priority === 'critical' ? '#ff4d6d' : c.priority === 'important' ? '#F7931A' : '#5a5a6e';
                  const failBg  = c.priority === 'critical' ? 'rgba(255,77,109,0.06)' : c.priority === 'important' ? 'rgba(247,147,26,0.05)' : 'rgba(90,90,110,0.04)';
                  const borderL = !c.pass ? `3px solid ${failCol}` : '3px solid transparent';
                  return (
                    <div key={c.label} style={{display:'flex', justifyContent:'space-between', alignItems:'center',
                      padding:'7px 10px 7px 8px', borderRadius:5,
                      background: c.pass ? 'transparent' : failBg,
                      borderLeft: borderL, transition:'background 0.15s'}}>
                      <div style={{display:'flex', gap:8, alignItems:'flex-start'}}>
                        <span style={{fontSize:13, color: c.pass ? '#00d084' : failCol, marginTop:1, flexShrink:0, fontWeight:700}}>{c.pass?'✓':'✗'}</span>
                        <div>
                          <div style={{fontSize:12, color: c.pass ? '#5a5a6e' : '#ece9e2', fontWeight: !c.pass && c.priority === 'critical' ? 700 : 400}}>{c.label}</div>
                      ...
                      <span style={{fontSize:11, fontFamily:"'JetBrains Mono',monospace", flexShrink:0, marginLeft:12,
                        color: c.pass ? '#3e3e52' : failCol}}>{c.val}</span>
                    </div>
                  );
                })}
              </div>
        ...
        <div style={scStyles.card}>
          <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:4}}>
            ...
            {sig && (
              <button onClick={() => {
                setEntryPrice(sig.price); setSlPrice(sig.sl); setTpPrice(sig.tp);
                setTier(sig.tier); setRiskPct(parseFloat(tierPct(sig.tier))); setCustomRisk(false);
              }} style={{...scStyles.loadBtn, marginTop:0, width:'auto', padding:'6px 14px', fontSize:11}}>
                ↑ Load T{sig.tier} · {sig.setup_type}
              </button>
            )}
          </div>
