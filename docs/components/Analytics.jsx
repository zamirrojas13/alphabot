...
// Strategy names come from window.STRAT_NAMES (set by Overview.jsx — single source of truth)
const _AN_SN = () => window.STRAT_NAMES || {};
...
function Analytics({ data, viewMode, setViewMode, dateRange }) {
  ...
  closed.forEach(t => {
    const g = t.grade || '—';
    if (!gradeR[g]) gradeR[g] = [];
    gradeR[g].push(t.r_multiple);
  });
      ...
      <div style={anStyles.twoCol}>
                    ...
                    return (
                      <div key={date}
                        ...
                        style={{ width:28, height:28, borderRadius:5, background:heatColor(val),
                          border: isToday ? '2px solid #F7931A' : '1px solid rgba(255,255,255,0.04)',
                          cursor:'default', display:'flex', alignItems:'center', justifyContent:'center',
                          boxShadow: isToday ? '0 0 6px rgba(247,147,26,0.4)' : 'none' }}>
                        <span style={{fontSize:9, color: isToday ? '#F7931A' : 'rgba(255,255,255,0.5)', fontFamily:"'JetBrains Mono',monospace", fontWeight: isToday ? 700 : 400}}>
        ...
        <div style={anStyles.card}>
          ...
          {ev_by_setup.length > 0 ? (
            <div style={{marginTop:14, display:'flex', flexDirection:'column', gap:14}}>
              {ev_by_setup.map(s => {
                const name = _AN_SN()[s.type?.toLowerCase()] || s.type;
        ...
        <div style={anStyles.card}>
          ...
          {closed.length > 0 ? (
            <div style={{marginTop:12}}>
              {/* Bar area — fixed height so % labels always visible */}
              <div style={{display:'flex', gap:6, alignItems:'flex-end', height:80, padding:'0 2px'}}>
                {hourly_stats.map(h => {
                  ...
                  const barH = total ? Math.max(4, wr * 60) : 4;
                  ...
                  return (
                    <div key={h.hour} style={{flex:1, display:'flex', flexDirection:'column', alignItems:'center'}}>
                      <div style={{height:16, display:'flex', alignItems:'flex-end', justifyContent:'center', marginBottom:2}}>
                        <span style={{fontSize:9, color: total ? col : 'transparent', fontWeight:700, fontFamily:"'JetBrains Mono',monospace"}}>
                          {total ? `${(wr*100).toFixed(0)}%` : '—'}
                        </span>
                      </div>
                      <div style={{width:'58%', minWidth:10, height:barH, background: total ? col : '#1f1f28',
                        borderRadius:'3px 3px 0 0', opacity: total ? 0.85 : 0.35}}></div>
                    </div>
                  );
                })}
              </div>
              {/* Time + W/L labels row */}
              <div style={{display:'flex', gap:6, borderTop:'1px solid #1a1a22', padding:'5px 2px 0'}}>
                {hourly_stats.map(h => {
                  const total = h.wins + h.losses;
                  return (
                    <div key={h.hour} style={{flex:1, textAlign:'center'}}>
                      <div style={{fontSize:9, color:'#3e3e52', fontFamily:"'JetBrains Mono',monospace"}}>{h.label}</div>
                      <div style={{fontSize:8, color:'#2a2a38', marginTop:2}}>{total ? `${h.wins}W·${h.losses}L` : 'no data'}</div>
                    </div>
                  );
                })}
              </div>
...
const anStyles = {
  ...
  kpiVal: { fontSize:18, fontWeight:700, fontFamily:"'JetBrains Mono',monospace", overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' },
